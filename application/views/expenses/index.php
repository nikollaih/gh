                            ...
                            <div class="card" id="ticketsList">
                                <div class="card-header border-0">
                                    <div class="d-flex align-items-center">
                                        <h5 class="card-title mb-0 flex-grow-1">Listado de gastos</h5>
                                        <div class="flex-shrink-0">
                                            <div class="d-flex flex-wrap gap-2">
                                                <button class="btn btn-success add-btn btn-add-expense pointer"><i class="mdi mdi-cash-plus align-bottom me-1"></i> Agregar</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="table-responsive table-card mb-4">
                                            ...
                                            <tbody class="list form-check-all" id="ticket-list-data">
                                                <?php
                                                    foreach ($expenses as $expense) { ?>
                                                        <tr id="expense-<?= $expense["id_expense"] ?>">
                                                            <td class="id"><?= $expense["id_expense"] ?></td>
                                                            <td class="tasks_name"><?= date("Y-m-d", strtotime($expense["date"])) ?><span style="opacity:0;font-size:1px;"><?= date("Ymd", strtotime($expense["date"])) ?></span></td>
                                                            <td class="category"><?= $expense["category"] ?></td>
                                                            <td class="client_name"><b>$<?= number_format($expense["price"], 0, ',', '.') ?></b><span style="opacity:0;font-size:1px;"><?= $expense["price"] ?></span></td>
                                                            <td class="assignedto"><?= $expense["description"] ?></td>
                                                            <td class="create_date"><?= date("Y-m-d h:i a", strtotime($expense["created_at"])) ?></td>
                                                            <td>
                                                                <ul class="list-inline hstack gap-2 mb-0">
                                                                    <li class="pointer list-inline-item edit btn-edit-expense" data-expense-id="<?= $expense["id_expense"] ?>" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Edit">
                                                                        <a class="text-primary d-inline-block edit-item-btn">
                                                                            <i class="ri-pencil-fill fs-16"></i>
                                                                        </a>
                                                                    </li>
                                                                    <li class="pointer list-inline-item btn-delete-expense" data-edit-id="<?= $expense["id_expense"] ?>" title="Remove">
                                        ...
                                        <div class="noresult" style="display: none">
                                            <div class="text-center">
                                                ...
                                                <p class="text-muted mb-0">Buscamos en <?= count($expenses) ?> movimientos pero no encontramos lo que buscas!</p>
                                            </div>
                                        </div>
                                    </div>
